...
DEFAULT_QUIC_PORT = 443  # Default port number for QUIC

# These variables may be modified by command line arguments
...
def setup_chrome_driver(enable_quic=True, domain=None):
    """
    ...
    if enable_quic:
        ...
        if domain:
            chrome_options.add_argument(f'--origin-to-force-quic-on={domain}:{quic_port}')
    else:
...
def get_timing_metrics(driver, domain, protocol, domain_id):
    """
    ...
    while True:
        try:
            print(f"Testing {url} with {protocol} (Attempt {attempt+1}/{MAX_RETRIES+1 if not (protocol == 'HTTP/3' and UNLIMITED_HTTP3_RETRIES) else 'unlimited'})...")
            driver.get(url)
    ...
    try:
        ...
        with open(output_file, 'w', newline='') as csvfile:
            ...
            for i, domain_info in enumerate(domains):
                ...
                # Create a driver with appropriate settings for the protocol
                if protocol == 'HTTP/3':
                    # For HTTP/3, use the origin-to-force-quic-on flag
                    driver = setup_chrome_driver(enable_quic=True, domain=domain)
                    try:
                        result = get_timing_metrics(driver, domain, protocol, domain_id)
                        
                        # Check if we successfully got HTTP/3
                        if 'h3' in result['nextHopProtocol'].lower() and result['successful']:
                            print(f"Successfully connected to {domain} using HTTP/3")
                        elif not UNLIMITED_HTTP3_RETRIES:
                            print(f"Failed to connect with HTTP/3 to {domain}")
                    finally:
                        driver.quit()
                    
                    # If we failed to get a result
                    if not result:
                        print(f"Could not connect to {domain} with HTTP/3")
...
def main():
    ...
    parser.add_argument('--quic-port', type=int, default=DEFAULT_QUIC_PORT,
                      help=f'Port number for QUIC connections (default: {DEFAULT_QUIC_PORT})')
    
    args = parser.parse_args()
    
    # Set global variables based on arguments
    global UNLIMITED_HTTP3_RETRIES, quic_port
    ...
    print(f"Using QUIC port: {quic_port}")
